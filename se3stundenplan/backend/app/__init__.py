        ...
        if path != "" and os.path.exists(app.static_folder + '/' + path):
            ...
            return jsonify({
                ...
                'api_endpoints': {
                    ...
                    'courses': '/api/courses/*',
                    'course_catalog': '/api/course-catalog/*',
                    'notifications': '/api/notifications/*',
                    'import_export': '/api/data/*',
                    'health': '/api/health'
                },
                'new_features': {
                    'course_selection': 'Kursauswahl aus Katalog verfügbar',
                    'multiple_sessions': 'Mehrere Termine pro Kurs unterstützt',
                    'conflict_detection': 'Automatische Zeitkonflikt-Erkennung'
                },
...
def register_blueprints(app):
    ...
    try:
        ...
        # Course Management (Legacy)
        from app.routes.courses import courses_bp
        app.register_blueprint(courses_bp, url_prefix='/api/courses')
        print("✅ Course Routes geladen")
        
        # Course Catalog & Selection (NEW)
        from app.routes.course_catalog_routes import course_catalog_bp
        app.register_blueprint(course_catalog_bp, url_prefix='/api/course-catalog')
        print("✅ Course Catalog Routes geladen")
...
def register_error_handlers(app):
    ...
    def missing_token(err_str):
        app.logger.error(f"[JWT] no token / malformed: {err_str}")
        return jsonify({"error":"token missing"}), 401

    @jwt.invalid_token_loader
    def invalid_token(err_str):
        app.logger.error(f"[JWT] invalid token: {err_str}")
        return jsonify({"error":"invalid token","message":err_str}), 422

    @jwt.expired_token_loader
    def expired_token(jwt_header, jwt_payload):
        msg = "expired at " + str(jwt_payload.get("exp"))
        app.logger.error(f"[JWT] expired: {msg}")
        return jsonify({"error":"token expired","message":msg}), 401
